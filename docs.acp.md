...
This document describes how the SKYKOI ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`SKYKOI acp` exposes an ACP agent over stdio and forwards prompts to a running
SKYKOI Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `SKYKOI acp` and speaks ACP messages over stdio.
...
`SKYKOI acp` resolves the Gateway URL and auth from CLI flags or config:
