...
// NOTE: This extension is intended to be bundled with SkyKoi.
// When running from source (tests/dev), SkyKoi internals live under src/.
...
export function createLlmTaskTool(api: SkyKoiPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via SkyKoi.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SkyKoi-llm-task-"));
