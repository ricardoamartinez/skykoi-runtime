...
IMAGE_NAME="SKYKOI-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="SKYKOI-net-e2e-$$"
GW_NAME="SKYKOI-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SKYKOI_GATEWAY_TOKEN=$TOKEN" \
	  -e "SKYKOI_SKIP_CHANNELS=1" \
	  -e "SKYKOI_SKIP_GMAIL_WATCHER=1" \
	  -e "SKYKOI_SKIP_CRON=1" \
	  -e "SKYKOI_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
