...
# Build and bundle SKYKOI into a minimal .app we can open.
# Outputs to dist/SKYKOI.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SKYKOI.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SKYKOI"
BUNDLE_ID="${BUNDLE_ID:-ai.SKYKOI.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/SKYKOI/SKYKOI/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SKYKOI/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SKYKOIBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SKYKOIGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SKYKOI"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SKYKOI"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SKYKOI"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SKYKOI" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SKYKOI/Resources/SKYKOI.icns" "$APP_ROOT/Contents/Resources/SKYKOI.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SKYKOI/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SKYKOIKit resources"
SKYKOIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SKYKOIKit_SKYKOIKit.bundle"
if [ -d "$SKYKOIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SKYKOIKit_SKYKOIKit.bundle"
  cp -R "$SKYKOIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SKYKOIKit_SKYKOIKit.bundle"
else
  echo "WARN: SKYKOIKit resource bundle not found at $SKYKOIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SKYKOI"
killall -q SKYKOI 2>/dev/null || true
