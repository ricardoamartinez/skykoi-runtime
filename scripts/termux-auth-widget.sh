...
SERVER="${SKYKOI_SERVER:-${SKYKOI_SERVER:-l36}}"

# Check auth status
termux-toast "Checking SKYKOI auth..."

STATUS=$(ssh "$SERVER" '$HOME/SKYKOI/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/SKYKOI/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SKYKOI_EXPIRING|SKYKOI_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/SKYKOI/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SKYKOI_EXPIRED|SKYKOI_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: SKYKOI won't work until re-auth"
                ;;
        esac
        ;;
