...
#
# Suggested cron: */30 * * * * /home/admin/SKYKOI/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SKYKOI notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., SKYKOI-alerts)
...
STATE_FILE="$HOME/.SKYKOI/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SKYKOI if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use SKYKOI
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SKYKOI to $NOTIFY_PHONE..."
            SKYKOI send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SKYKOI is down. Run: ssh l36 '~/SKYKOI/scripts/mobile-reauth.sh'" "urgent"
    exit 1
