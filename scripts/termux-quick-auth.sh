...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/SKYKOI/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SKYKOI Re-Auth" -c "After getting key, run: ssh $SERVER '~/SKYKOI/scripts/mobile-reauth.sh'" --id SKYKOI-auth
        ;;
    *)
