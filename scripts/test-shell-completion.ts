 ...
 * `SKYKOI update`. Use it to verify the completion installation flow
...
\`SKYKOI update\`, \`SKYKOI doctor\`, and \`SKYKOI onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SKYKOI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`SKYKOI completion --install\` later to enable.`));
    return;
  }
