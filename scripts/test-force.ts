...
function runTests() {
  const isolatedLock =
    process.env.SKYKOI_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `SKYKOI-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      SKYKOI_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.SKYKOI_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
