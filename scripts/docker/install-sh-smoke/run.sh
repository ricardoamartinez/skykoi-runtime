...
INSTALL_URL="${SKYKOI_INSTALL_URL:-https://SKYKOI.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SKYKOI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SKYKOI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="SKYKOI"
PACKAGE_NAME="${SKYKOI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SKYKOI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SKYKOI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SKYKOI_INSTALL_LATEST_OUT:-}"
fi
