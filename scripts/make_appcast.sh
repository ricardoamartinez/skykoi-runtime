...
ZIP=${1:?"Usage: $0 SKYKOI-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/SKYKOI/SKYKOI/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^SKYKOI-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/SKYKOI/SKYKOI/releases/download/v${VERSION}/"}
