...
BASE_IMAGE="${BASE_IMAGE:-SKYKOI-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-SKYKOI-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=SKYKOI.sandbox=1)
NOTE
