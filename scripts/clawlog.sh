...
show_usage() {
    cat << EOF
clawlog - SKYKOI Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View SKYKOI logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    SKYKOI logs flow through the macOS unified log (subsystem: ai.SKYKOI).
