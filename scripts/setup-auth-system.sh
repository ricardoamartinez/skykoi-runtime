#!/bin/bash
# Setup SKYKOI Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'SKYKOI-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SKYKOI message: Send warning via SKYKOI itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/SKYKOI-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/SKYKOI-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/SKYKOI-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now SKYKOI-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status SKYKOI-auth-monitor.timer"
echo ""
echo "Quick commands:"
