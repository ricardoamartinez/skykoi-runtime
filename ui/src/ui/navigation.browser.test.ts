...
import { SKYKOIApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = SKYKOIApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("SKYKOI-app") as SKYKOIApp;
...
beforeEach(() => {
  SKYKOIApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SKYKOI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SKYKOIApp.prototype.connect = originalConnect;
  window.__SKYKOI_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/SKYKOI/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/SKYKOI");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/SKYKOI/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SKYKOI_CONTROL_UI_BASE_PATH__ = "/SKYKOI";
    const app = mountApp("/SKYKOI/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/SKYKOI");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/SKYKOI/sessions");
  });
