...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `SKYKOI-e2e-${name}-`));
  const configDir = path.join(homeDir, ".SKYKOI");
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SKYKOI_GATEWAY_TOKEN: inst.gatewayToken,
        SKYKOI_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SKYKOI_GATEWAY_PORT: String(gwA.port),
          SKYKOI_GATEWAY_TOKEN: gwA.gatewayToken,
          SKYKOI_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SKYKOI_GATEWAY_PORT: String(gwB.port),
          SKYKOI_GATEWAY_TOKEN: gwB.gatewayToken,
          SKYKOI_GATEWAY_PASSWORD: "",
        }),
