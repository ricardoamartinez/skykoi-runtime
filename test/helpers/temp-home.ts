...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SKYKOI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SKYKOI_STATE_DIR = path.join(base, ".SKYKOI");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "SKYKOI-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".SKYKOI", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
