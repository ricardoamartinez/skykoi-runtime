...
import type { SKYKOIConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("SKYKOI-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("SKYKOI-bin-sherpa-");
      const modelDir = await makeTempDir("SKYKOI-sherpa-model-");
      ...
      const cfg: SKYKOIConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("SKYKOI-bin-whispercpp-");
      const modelDir = await makeTempDir("SKYKOI-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("SKYKOI-bin-gemini-");
      ...
      const cfg: SKYKOIConfig = { tools: { media: { image: {} } } };
