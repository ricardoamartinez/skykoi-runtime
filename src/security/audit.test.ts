...
describe("security audit", () => {
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: SKYKOIConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.SKYKOI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-security-audit-discord-"));
    process.env.SKYKOI_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.SKYKOI_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "SKYKOI-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.SKYKOI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.SKYKOI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-security-audit-discord-open-"));
    process.env.SKYKOI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SKYKOIConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.SKYKOI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-security-audit-slack-"));
    process.env.SKYKOI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SKYKOIConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.SKYKOI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-security-audit-slack-open-"));
    process.env.SKYKOI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SKYKOIConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.SKYKOI_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-security-audit-telegram-"));
    process.env.SKYKOI_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: SKYKOIConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: SKYKOIConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    ...
    process.env.SKYKOI_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: SKYKOIConfig = {
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: SKYKOIConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-audit-scanner-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        SKYKOI: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    it("prefers env token over local config token", async () => {
      process.env.SKYKOI_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SKYKOIConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.SKYKOI_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: SKYKOIConfig = {
        gateway: {
          mode: "remote",
