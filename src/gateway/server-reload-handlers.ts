  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue((process.env.SKYKOI_SKIP_GMAIL_WATCHER))) {
        ...
        params.logHooks.info("skipping gmail watcher restart (SKYKOI_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (SKYKOI_SKIP_CHANNELS=1 (or SKYKOI_SKIP_CHANNELS=1))",
        );
      } else {
