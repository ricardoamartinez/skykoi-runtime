...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SKYKOI_SKIP_CHANNELS;
    prevSkipGmail = process.env.SKYKOI_SKIP_GMAIL_WATCHER;
    process.env.SKYKOI_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SKYKOI_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SKYKOI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SKYKOI_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
