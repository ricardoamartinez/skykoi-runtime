...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.SKYKOI_CONFIG_PATH;
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.SKYKOI_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.SKYKOI_DISABLE_CONFIG_CACHE;
    } else {
      process.env.SKYKOI_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
