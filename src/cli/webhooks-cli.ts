...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.SKYKOI.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SKYKOI hooks")
    ...
    .option("--hook-url <url>", "SKYKOI hook URL")
    .option("--hook-token <token>", "SKYKOI hook token")
