...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "SkyKoi-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.SKYKOI_STATE_DIR;
    prevBundledDir = process.env.SKYKOI_BUNDLED_HOOKS_DIR;
    process.env.SKYKOI_STATE_DIR = path.join(baseDir, "state");
    process.env.SKYKOI_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.SKYKOI_BUNDLED_HOOKS_DIR;
    } else {
      process.env.SKYKOI_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
