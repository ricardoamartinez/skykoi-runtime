...
import { resolveSKYKOIDocsPath } from "./docs-path.js";
...
export async function runCliAgent(params: {
  ...
  const heartbeatPrompt =
    sessionAgentId === defaultAgentId
      ...
      : undefined;
  const docsPath = await resolveSKYKOIDocsPath({
    workspaceDir,
  ...
  try {
    const output = await enqueueCliRun(queueKey, async () => {
      ...
      const logOutputText = isTruthyEnvValue(process.env.SKYKOI_CLAUDE_CLI_LOG_OUTPUT);
      if (logOutputText) {
