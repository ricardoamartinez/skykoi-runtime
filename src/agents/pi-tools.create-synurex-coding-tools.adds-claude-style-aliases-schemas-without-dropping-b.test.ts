...
describe("createSKYKOICodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createSKYKOICodingTools({
      config,
    ...
    const anthropicTools = createSKYKOICodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createSKYKOICodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createSKYKOICodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
