...
describe("oauth paths", () => {
  it("prefers SKYKOI_OAUTH_DIR over SKYKOI_STATE_DIR", () => {
    const env = {
      SKYKOI_OAUTH_DIR: "/custom/oauth",
  ...
  it("derives oauth path from SKYKOI_STATE_DIR when unset", () => {
    const env = {
      SKYKOI_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("state + config path candidates", () => {
  it("uses SKYKOI_STATE_DIR when set", () => {
    const env = {
      SKYKOI_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;
  ...
  it("CONFIG_PATH prefers existing config when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "SKYKOI-config-"));
    ...
    try {
      ...
      if (previousSKYKOIConfig === undefined) {
        delete process.env.SKYKOI_CONFIG_PATH;
      } else {
        process.env.SKYKOI_CONFIG_PATH = previousSKYKOIConfig;
      }
      if (previousSKYKOIConfig === undefined) {
        delete process.env.SKYKOI_CONFIG_PATH;
      } else {
      ...
      if (previousSKYKOIState === undefined) {
        delete process.env.SKYKOI_STATE_DIR;
      } else {
        process.env.SKYKOI_STATE_DIR = previousSKYKOIState;
      }
      if (previousSKYKOIState === undefined) {
        delete process.env.SKYKOI_STATE_DIR;
      } else {
