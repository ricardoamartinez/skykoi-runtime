...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-SKYKOI",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-SKYKOI#general");
  });
  ...
  it("derives session transcripts dir from SKYKOI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SKYKOI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.SKYKOI_STATE_DIR = "/custom/state";
    try {
