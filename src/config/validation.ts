...
import type { SkyKoiConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SkyKoiConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SkyKoiConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SkyKoiSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SkyKoiConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SkyKoiConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SkyKoiConfig)),
    ),
  };
}
