...
import { resolveSKYKOIPackageRoot, resolveSKYKOIPackageRootSync } from "./SKYKOI-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSKYKOIPackageRoot({ argv1: normalized });
  if (packageRoot) {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveSKYKOIPackageRootSync({
    argv1,
