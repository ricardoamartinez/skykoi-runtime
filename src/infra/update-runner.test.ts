...
describe("runGatewayUpdate", () => {
  ...
  it("updates global npm installs when detected", async () => {
    ...
    expect(calls.some((call) => call === "npm i -g SKYKOI@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g SKYKOI@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g SKYKOI@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a SKYKOI checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-SKYKOI-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when SKYKOI.mjs is missing", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "skykoi", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
    ...
    expect(result.steps.at(-1)?.name).toBe("SKYKOI doctor entry");
  });
