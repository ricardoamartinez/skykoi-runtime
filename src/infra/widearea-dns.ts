...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SKYKOI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*SKYKOI-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  records.push(`_SKYKOI-gw._tcp IN PTR ${instanceLabel}._SKYKOI-gw._tcp`);
  records.push(`${instanceLabel}._SKYKOI-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._SKYKOI-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; SKYKOI-content-hash: ${contentHash}\n${contentBody}`;
}
