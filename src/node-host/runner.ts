...
const execHostEnforced = process.env.SKYKOI_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.SKYKOI_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSkyKoiCliOnPath({ pathEnv: process.env.PATH ?? "" });
