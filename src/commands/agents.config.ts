...
export function listAgentEntries(cfg: SKYKOIConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SKYKOIConfig, agentId: string) {
...
function resolveAgentModel(cfg: SKYKOIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SKYKOIConfig): AgentSummary[] {
