    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from SKYKOI.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "SKYKOI update wizard" : "SKYKOI configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: SKYKOIConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("SKYKOI doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? (process.env.SKYKOI_GATEWAY_TOKEN);
        const password =
          nextConfig.gateway?.auth?.password ?? (process.env.SKYKOI_GATEWAY_PASSWORD);
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? (process.env.SKYKOI_GATEWAY_PASSWORD);
    const oldPassword = baseConfig.gateway?.auth?.password ?? (process.env.SKYKOI_GATEWAY_PASSWORD);
    const token = nextConfig.gateway?.auth?.token ?? (process.env.SKYKOI_GATEWAY_TOKEN);
