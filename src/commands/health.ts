...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SKYKOI_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: SkyKoiConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SKYKOI_DEBUG_HEALTH);
    if (opts.verbose) {
