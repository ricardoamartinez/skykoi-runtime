...
function captureEnv(): EnvSnapshot {
  return {
    ...
    disableConfigCache: process.env.SKYKOI_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("SKYKOI_STATE_DIR", prev.stateDir);
  restoreEnvVar("SKYKOI_CONFIG_PATH", prev.configPath);
  restoreEnvVar("SKYKOI_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("SKYKOI_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("SKYKOI_SKIP_CRON", prev.skipCron);
  restoreEnvVar("SKYKOI_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("SKYKOI_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("SKYKOI_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("SKYKOI_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("SkyKoi-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("SkyKoi-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("SkyKoi-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("SkyKoi-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("SkyKoi-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("SkyKoi-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
