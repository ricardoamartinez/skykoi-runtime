import type { BrowserConfig, BrowserProfileConfig, SKYKOIConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_SKYKOI_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_SKYKOI_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SKYKOI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SKYKOI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SKYKOI browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for SKYKOI/SKYKOI CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SKYKOIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SKYKOI_BROWSER_ENABLED;
