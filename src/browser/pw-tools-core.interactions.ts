  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-SkyKoi-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-SkyKoi-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-SkyKoi-labels", "1");
          ...
          tag.setAttribute("data-SkyKoi-labels", "1");
