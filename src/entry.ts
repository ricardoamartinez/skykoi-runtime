...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.SKYKOI_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.SKYKOI_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.SKYKOI_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[SKYKOI] ${parsed.error}`);
    process.exit(2);
  }
