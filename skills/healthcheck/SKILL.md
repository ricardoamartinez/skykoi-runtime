---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SkyKoi deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SkyKoi cron scheduling for periodic checks, or version status checks on a machine running SkyKoi (laptop, workstation, Pi, VPS).
---

# SkyKoi Host Hardening

## Overview

Assess and harden the host running SkyKoi, then align it to a user-defined risk tolerance without breaking access. Use SkyKoi security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SkyKoi changes the host firewall, SSH, or OS updates; it does not.
...
5. SkyKoi gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SkyKoi can access sensitive data.
...
### 2) Run SkyKoi security audits (read-only)

As part of the default read-only checks, run `SkyKoi security audit --deep`. Only offer alternatives if the user requests them:

1. `SkyKoi security audit` (faster, non-probing)
2. `SkyKoi security audit --json` (structured output)

Offer to apply SkyKoi safe defaults (numbered):

1. `SkyKoi security audit --fix`

Be explicit that `--fix` only tightens SkyKoi defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SkyKoi version/update status (read-only)

As part of the default read-only checks, run `SkyKoi update status`.
...
- Credential hygiene notes (location of SkyKoi creds, prefer disk encryption)
...
After SkyKoi install or first hardening pass, run at least one baseline audit and version check:

- `SkyKoi security audit`
- `SkyKoi security audit --deep`
- `SkyKoi update status`

Ongoing monitoring is recommended. Use the SkyKoi cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `SkyKoi cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `SkyKoi update status`
...
Before creating, `SkyKoi cron list` and match on exact `name`. If found, `SkyKoi cron edit <id> ...`.
If not found, `SkyKoi cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `SkyKoi update status` (preferred for source checkouts and channels)
2. `npm view SkyKoi version` (published npm version)

## SkyKoi command accuracy

Use only supported commands and flags:

- `SkyKoi security audit [--deep] [--fix] [--json]`
- `SkyKoi status` / `SkyKoi status --deep`
- `SkyKoi health --json`
- `SkyKoi update status`
- `SkyKoi cron add|list|runs|run`

Do not invent CLI flags or imply SkyKoi enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SkyKoi compaction:
