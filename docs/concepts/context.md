...
read_when:
  - You want to understand what “context” means in SKYKOI
...
“Context” is **everything SKYKOI sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (SKYKOI-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How SKYKOI builds the system prompt

The system prompt is **SKYKOI-owned** and rebuilt each run. It includes:
...
By default, SKYKOI injects a fixed set of workspace files (if present):
