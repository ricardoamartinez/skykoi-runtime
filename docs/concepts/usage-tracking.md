...
- `/usage cost` in chats: local cost summary aggregated from SKYKOI session logs.
- CLI: `SKYKOI status --usage` prints a full per-provider breakdown.
- CLI: `SKYKOI channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
