---
summary: "Use MiniMax M2.1 in SkyKoi"
read_when:
  - You want MiniMax models in SkyKoi
...
```bash
SkyKoi plugins enable minimax-portal-auth  # skip if already loaded.
SkyKoi gateway restart  # restart if gateway is already running
SkyKoi onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/SkyKoi/SkyKoi/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `skykoi.json`:
...
- Use `SkyKoi models list` and `SkyKoi models set minimax/MiniMax-M2.1` to switch.
...
- Running `SkyKoi configure` and selecting **MiniMax M2.1**, or
