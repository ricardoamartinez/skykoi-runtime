...
read_when:
  - Designing workspace memory (~/.SKYKOI/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SKYKOI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.SKYKOI/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.SKYKOI/workspace` as canonical human-readable memory.
...
```
~/.SKYKOI/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SKYKOI**, but keep a separable core library.

### Why integrate into SKYKOI?

- SKYKOI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `SKYKOI memory recall "…" --k 25 --since 30d`
  - `SKYKOI memory reflect --since 7d`
...
Pragmatic take for `~/.SKYKOI/workspace`:
