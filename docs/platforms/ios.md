...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `SKYKOI.internal.`), **or**
...
The Gateway advertises `_SKYKOI-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `SKYKOI.internal.`) and Tailscale split DNS.
...
- The Gateway canvas host serves `/__SKYKOI__/canvas/` and `/__SKYKOI__/a2ui/`.
...
- Pairing prompt never appears: run `SKYKOI nodes pending` and approve manually.
