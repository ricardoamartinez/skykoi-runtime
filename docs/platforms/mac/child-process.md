...
service via the external `SkyKoi` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SKYKOI_PROFILE`; legacy `com.skykoi.*` is supported).
