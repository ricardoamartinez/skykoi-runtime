...
```bash
SKYKOI tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `SKYKOI tui --deliver`
...
- Check the Gateway logs: `SKYKOI logs --follow`.
- Confirm the agent can run: `SKYKOI status` and `SKYKOI models status`.
...
- No agents in picker: check `SKYKOI agents list` and your routing config.
