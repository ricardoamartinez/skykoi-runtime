...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/SkyKoi`)
...
If the page fails to load, start the Gateway first: `SkyKoi gateway`.
...
you revoke it with `SkyKoi devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.skykoi/skykoi.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SkyKoi
...
SkyKoi **blocks** Control UI connections without device identity.
