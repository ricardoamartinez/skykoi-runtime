...
Tlon is a decentralized messenger built on Urbit. SKYKOI connects to your Urbit ship and can
...
```bash
SKYKOI plugins install @SKYKOI/tlon
```

Local checkout (when running from a git repo):

```bash
SKYKOI plugins install ./extensions/tlon
```
...
Use these with `SKYKOI message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SKYKOI replies in-thread.
