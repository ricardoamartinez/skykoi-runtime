...
Matrix is an open, decentralized messaging protocol. SkyKoi connects as a Matrix **user**
...
```bash
SkyKoi plugins install @SkyKoi/matrix
```

Local checkout (when running from a git repo):

```bash
SkyKoi plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `SkyKoi plugins install @SkyKoi/matrix`
   - From a local checkout: `SkyKoi plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SkyKoi calls the same
     login endpoint, stores the access token in `~/.skykoi/credentials/matrix/credentials.json`,
...
- On first connection, SkyKoi requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SkyKoi logs a warning.
...
`~/.skykoi/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `SkyKoi pairing list matrix`
  - `SkyKoi pairing approve matrix <CODE>`
...
- On startup, SkyKoi resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
