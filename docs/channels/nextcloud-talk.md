...
```bash
SKYKOI plugins install @SKYKOI/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
SKYKOI plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SKYKOI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `SKYKOI pairing list nextcloud-talk`
  - `SKYKOI pairing approve nextcloud-talk <CODE>`
