...
read_when:
  - Setting up Twitch chat integration for SkyKoi
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SkyKoi connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
SkyKoi plugins install @SkyKoi/twitch
```

Local checkout (when running from a git repo):

```bash
SkyKoi plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SKYKOI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "SkyKoi", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SKYKOI_TWITCH_ACCESS_TOKEN env var)
...
```bash
SKYKOI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
