...
SkyKoi can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, SkyKoi retries the same message as plain text.

## Commands (native + custom)

SkyKoi registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; SkyKoi does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `SkyKoi logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). SkyKoi can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. SkyKoi:
...
Private chats can include `message_thread_id` in some edge cases. SkyKoi keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `SkyKoi logs --follow` and look for `from.id`.
...
SkyKoi defaults to audio files for backward compatibility.
...
SkyKoi supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, SkyKoi handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, SkyKoi caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.skykoi/telegram/sticker-cache.json`
...
SkyKoi uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, SkyKoi:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by SkyKoi)
...
- Example: `SkyKoi message send --channel telegram --target 123456789 --message "hi"`.
...
- `SkyKoi channels status` shows a warning when config expects unmentioned group messages.
- `SkyKoi channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `SkyKoi logs --follow` (look for "skipping group message")
...
- Upgrade to a SkyKoi build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
