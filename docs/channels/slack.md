...
3. Set tokens for SkyKoi and start the gateway.
...
7. Slash Commands → create `/SkyKoi` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
SkyKoi can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, SkyKoi falls back to the user token.
...
```json
{
  "display_information": {
    "name": "SkyKoi",
    "description": "Slack connector for SkyKoi"
  },
  "features": {
    "bot_user": {
      "display_name": "SkyKoi",
    ...
    "slash_commands": [
      {
        "command": "/SkyKoi",
        "description": "Send a message to SkyKoi",
        "should_escape": false
      }
    ]
  },
...
By default, SkyKoi replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, SkyKoi infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `SkyKoi pairing approve slack <code>`.
...
- On startup, SkyKoi resolves channel/user names in allowlists to IDs (when tokens allow)
