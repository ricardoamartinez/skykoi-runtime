...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.skykoi/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.skykoi/node.json` (node id + pairing token).
...
Path: `~/.skykoi/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.skykoi/exec-approvals.sock",
...
- Unix socket at `~/.skykoi/exec-approvals.sock` (0600).
