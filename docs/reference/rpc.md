...
SKYKOI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SKYKOI owns lifecycle when `channels.signal.autoStart=true`.
...
- SKYKOI spawns `imsg rpc` as a child process (legacy iMessage integration).
