...
read_when:
  - You need detailed behavior for SkyKoi onboard
...
This page is the full reference for `SkyKoi onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.skykoi/skykoi.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `SkyKoi doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `SkyKoi pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `SkyKoi health`.
    - `SkyKoi status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.skykoi/.env` so launchd can read it.
...
- OAuth credentials: `~/.skykoi/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.skykoi/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`SkyKoi onboard`](/cli/onboard)
