---
title: Lobster
summary: "Typed workflow runtime for SKYKOI with resumable approval gates."
description: Typed workflow runtime for SKYKOI — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets SKYKOI run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: SKYKOI runs one Lobster tool call and gets a structured result.
...
SKYKOI launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | SKYKOI.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
SKYKOI.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In SKYKOI tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the SKYKOI Gateway (see the [Lobster repo](https://github.com/SKYKOI/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), SKYKOI keeps core tools enabled. To restrict core
...
→ SKYKOI calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ SKYKOI calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls SKYKOI tools that do.
