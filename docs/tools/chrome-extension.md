---
summary: "Chrome extension: let SkyKoi drive your existing Chrome tab"
read_when:
...
The SkyKoi Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate SkyKoi-managed Chrome profile.
...
SkyKoi then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SkyKoi release (npm package) as static files. There is no separate “build” step.

After upgrading SkyKoi:

- Re-run `SkyKoi browser extension install` to refresh the installed files under your SkyKoi state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

SkyKoi ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want SkyKoi to control.
...
- `ON`: attached; SkyKoi can drive that tab.
...
Debugging: `SkyKoi sandbox explain`
...
`SkyKoi browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `SkyKoi browser extension install` first to copy the extension to a stable location under your SkyKoi state directory.
...
- **This is not isolated** like the dedicated SkyKoi-managed profile.
