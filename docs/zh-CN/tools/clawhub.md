---
read_when:
  - 向新用户介绍 SkyKoi Skills
  - 安装、搜索或发布 Skills
  - 说明 SkyKoi Skills CLI 标志和同步行为
summary: SkyKoi Skills 指南：公共 Skills 注册中心 + CLI 工作流
title: SkyKoi Skills
x-i18n:
  ...
  source_path: tools/SkyKoi Skills.md
  workflow: 15
---

# SkyKoi Skills

SkyKoi Skills 是 **SkyKoi 的公共 Skills 注册中心**。它是一项免费服务：所有 Skills 都是公开的、开放的，所有人都可以查看、共享和复用。Skills 就是一个包含 `SKILL.md` 文件（以及辅助文本文件）的文件夹。你可以在网页应用中浏览 Skills，也可以使用 CLI 来搜索、安装、更新和发布 Skills。

网站：[SkyKoi Skills.com](https://skykoi.com/skills)

## 适用人群（新手友好）

如果你想为 SkyKoi 智能体添加新功能，SkyKoi Skills 是查找和安装 Skills 的最简单方式。你不需要了解后端的工作原理。你可以：
...
4. 启动一个新的 SkyKoi 会话，以加载新 Skills。
...
默认情况下，CLI 会将 Skills 安装到当前工作目录下的 `./skills`。如果已配置 SkyKoi 工作区，`SkyKoi Skills` 会回退到该工作区，除非你通过 `--workdir`（或 `SkyKoi Skills_WORKDIR`）进行覆盖。SkyKoi 从 `<workspace>/skills` 加载工作区 Skills，并会在**下一个**会话中生效。如果你已经在使用 `~/.skykoi/skills` 或内置 Skills，工作区 Skills 优先级更高。
...
- `--workdir <dir>`：工作目录（默认：当前目录；回退到 SkyKoi 工作区）。
...
- `SkyKoi Skills login`（浏览器流程）或 `SkyKoi Skills login --token <token>`
...
- `SkyKoi Skills list`（读取 `.SkyKoi Skills/lock.json`）
...
`SkyKoi Skills sync` 首先扫描当前工作目录。如果未找到 Skills，它会回退到已知的旧版位置（例如 `~/SkyKoi/skills` 和 `~/.skykoi/skills`）。这样设计是为了在不需要额外标志的情况下找到旧版 Skills 安装。

### 存储和锁文件

- 已安装的 Skills 记录在工作目录下的 `.SkyKoi Skills/lock.json` 中。
- 认证令牌存储在 SkyKoi Skills CLI 配置文件中（可通过 `SkyKoi Skills_CONFIG_PATH` 覆盖）。

### 遥测（安装计数）

当你在登录状态下运行 `SkyKoi Skills sync` 时，CLI 会发送一个最小快照用于计算安装次数。你可以完全禁用此功能：
...
- `SkyKoi Skills_SITE`：覆盖网站 URL。
- `SkyKoi Skills_REGISTRY`：覆盖注册中心 API URL。
- `SkyKoi Skills_CONFIG_PATH`：覆盖 CLI 存储令牌/配置的位置。
- `SkyKoi Skills_WORKDIR`：覆盖默认工作目录。
- `SkyKoi Skills_DISABLE_TELEMETRY=1`：禁用 `sync` 的遥测功能。
