...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SKYKOI
...
Running SKYKOI via Node with `tsx` fails at startup with:

```
[SKYKOI] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `SKYKOI status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch SKYKOI.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node SKYKOI.mjs status` works on Node 25.
