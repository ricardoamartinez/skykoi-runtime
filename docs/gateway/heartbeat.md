...
- During heartbeat runs, SkyKoi treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `SkyKoi sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SkyKoi skips the heartbeat run entirely (no model call).
...
```bash
SkyKoi system event --text "Check for urgent follow-ups" --mode now
```
