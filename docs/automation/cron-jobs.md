...
- Jobs persist under `~/.SKYKOI/cron/` so restarts don’t lose schedules.
...
SKYKOI cron list
SKYKOI cron run <job-id>
SKYKOI cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.SKYKOI/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `SKYKOI cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, SKYKOI defaults to `announce`.
...
- Job store: `~/.SKYKOI/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.SKYKOI/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
SKYKOI cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
SKYKOI cron edit <jobId> --agent ops
SKYKOI cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
SKYKOI cron run <jobId>
SKYKOI cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
SKYKOI cron edit <jobId> \
...
```bash
SKYKOI system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`SKYKOI system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SKYKOI_SKIP_CRON`.
